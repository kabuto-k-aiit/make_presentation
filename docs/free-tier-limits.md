...
### Google Gemini API
- **制限**: 
  - Gemini 1.5 Flash: 15RPM (1分間), 1000RPH (1時間)
  - Gemini 1.5 Pro: 2RPM, 50RPH
- **対策**:
  - バックエンドでレート制限 (5回/時間)
  - APIエラー時の自動リトライ
  - ユーザーへの使用制限通知
...
### アプリケーション内レート制限
- **プレゼンテーション生成**: 5回/時間/ユーザー（本番環境）
- **開発環境**: `DISABLE_RATE_LIMIT=true` で制限解除（1000回/時間）
- **目的**: APIコスト制御と公平な利用
- **実装**: FastAPI Limiter + Redis + 環境変数制御
